...
dataFile = 'ETHUSDT-1m-2024-05-22.csv'
...
def loadFinamCsv(fname):
    ...
    for line in fi:
        ...
        timeEpoch = parsed[0]
        #dtime = time.strptime(timeStamp, '%Y%m%d%H%M%S')
        #timeEpoch = time.mktime(dtime)

        bars[counter, :] = np.array((timeEpoch, np.float64(parsed[1]), np.float64(parsed[2]),
                                     np.float64(parsed[3]), np.float64(parsed[4]), np.float64(parsed[5])))
...
plt.errorbar(xs[blackBars], totalCentres[blackBars], yerr=totalSpans[blackBars], ecolor='gray', elinewidth=0.5, capsize=0,
             ls='none')
plt.errorbar(xs[blackBars], bodyCentres[blackBars], yerr=np.abs(bodySpans[blackBars]), ecolor='gray', elinewidth=0.5,
             capsize=0, ls='none')

plt.errorbar(xs[greenBars], totalCentres[greenBars], yerr=totalSpans[greenBars], ecolor='gray', elinewidth=0.5, capsize=0,
             ls='none')
plt.errorbar(xs[greenBars], bodyCentres[greenBars], yerr=np.abs(bodySpans[greenBars]), ecolor='gray', elinewidth=0.5,
             capsize=0, ls='none')

plt.errorbar(xs[redBars], totalCentres[redBars], yerr=totalSpans[redBars], ecolor='gray', elinewidth=0.5, capsize=0,
             ls='none')
plt.errorbar(xs[redBars], bodyCentres[redBars], yerr=np.abs(bodySpans[redBars]), ecolor='gray', elinewidth=0.5, capsize=0,
             ls='none')
